...
class KimiaNetEncoder:
    ...
    def __init__(self):
        ...
        self.intermediate_features_names = ["pool2_pool", "pool3_pool", "pool4_pool", "relu"]
...
class KimiaNetDecoder:
    ...
    def __init__(self):
        ...
        self.network_input_patch_width = network_input_patch_width
        self.img_format = img_format
        self.decoder_blocks = [16, 24, 12, 6]
    ...
    def create_decoder(self, encoder=None):
        if encoder is None:
            encoder = KimiaNetEncoder()
        bn_axis = 3 if backend.image_data_format() == "channels_last" else 1
        i = encoder.intermediate_features[-1]
        x = self.dense_block_transpose(i, self.decoder_blocks[0], "dense_block1")
        x = layers.Concatenate(axis=-1)([x, encoder.intermediate_features[-2]])
        x = self.transition_block_transpose(x, "transition_block1")
        ...
        x = layers.Concatenate(axis=-1)([x, encoder.intermediate_features[-4]])
        x = self.transition_block_transpose(x, "transition_block3")
        x = self.dense_block_transpose(x, self.decoder_blocks[3], "dense_block4")
...
class KimiaNetAutoencoder:
    ...
    def __init__(self):
        self.patch_dir = patch_dir
        self.network_weights_address = network_weights_address
        self.network_input_patch_size = network_input_patch_width
    ...
    def create_autoencoder(self):
        encoder = KimiaNetEncoder()
        decoder_model = KimiaNetDecoder().create_decoder(encoder=encoder)
        i = encoder.model.input
        x = encoder.model(i)
        x = decoder_model(x)
...
def train_autoencoder():
    ...
    callbacks_frozen = [
        tf.keras.callbacks.ModelCheckpoint(
            ...
            filepath=f"../models/Model_{batch_size}_{lr}_{date_time}" + "/1",
    ...
    callbacks = [
        tf.keras.callbacks.ModelCheckpoint(
            filepath=f"../models/Model_{batch_size}_{lr}_{date_time}" + "/2",
            save_best_only=True,  # Only save a model if `val_loss` has improved.
            monitor="val_loss",
            verbose=1,
        ),
